...
export default function Stats() {
  ...
  return (
    <>
      <Fonts />
      <section id="stats" className="w-full py-20 px-4 relative overflow-hidden">
        {/* Background decoration */}
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#00FF9E]/5 to-transparent" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
          {stats.map((stat, i) => (
            <motion.div key={i} className="relative group" variants={itemVariants}>
              {/* Glow effect */}
              <motion.div className="absolute inset-0 bg-[#00FF9E]/20 rounded-2xl blur-xl" variants={glowVariants} />
              ...
              <div className="relative bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-2xl p-8 text-center hover:border-[#00FF9E]/50 transition-all duration-500 group-hover:transform group-hover:scale-105">
                ...
                <motion.div
                  ...
                  transition={{ duration: 0.6, delay: i * 0.3 + 0.8 }}
                >
                  {stat.label}
                </motion.div>
                ...
                >
                  <motion.div
                    ...
                    initial={{ scaleX: 0 }}
                    animate={isInView ? { scaleX: 1 } : { scaleX: 0 }}
                    transition={{ duration: 1.5, delay: i * 0.3 + 1.2, ease: "easeOut" }}
