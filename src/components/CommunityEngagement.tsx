...
// Instagram embed fallback component
function InstagramEmbedWithFallback({ post }: { post: CommunityPost }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const container = document.querySelector(`[data-instgrm-permalink='${post.url}/']`);
      // If container is not rendered or is too small, show fallback
      if (!container || (container as HTMLElement).clientHeight < 100) {
  ...
  if (loading) {
    return (
      <div className="w-full h-[320px] flex items-center justify-center bg-gradient-to-br from-gray-800 via-gray-700 to-gray-900 animate-pulse rounded-xl">
        <div className="w-24 h-24 bg-gray-700 rounded-full blur-sm" />
      </div>
    );
  }

  // Fallback UI if embed not visible
  if (!embedVisible) {
    return (
      <div className="w-full h-[320px] flex flex-col items-center justify-center bg-gradient-to-br from-gray-800 via-gray-700 to-gray-900 rounded-xl">
        <div className="w-20 h-20 mb-4 rounded-full bg-gradient-to-tr from-pink-500 via-yellow-400 to-purple-500 blur-sm" />
        <p className="text-gray-300 text-lg font-light mb-2">Instagram reel preview unavailable</p>
        <a
          href={post.url}
          target="_blank"
  ...
  // If embed is visible, render it
  return (
    <div className="w-full flex justify-center instagram-embed-container" dangerouslySetInnerHTML={{ __html: post.embedCode }} />
  );
}
  ...
  const processInstagramEmbeds = () => {
    if (typeof window !== 'undefined' && window.instgrm) {
      try {
        ...
        console.log('Instagram embed processing failed:', error);
      }
    }
  };

  useEffect(() => {
    // Process embeds after component mounts and scripts are loaded
    const timer = setTimeout(() => {
    ...
    <>
      ...
      <section className="py-32 px-6 forest-bg">
        <div className="max-w-7xl mx-auto">
          ...
          <div ref={containerRef} className="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-6xl mx-auto mb-16">
