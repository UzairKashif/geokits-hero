...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      material.uniforms.uRot.value.set(cos, sin);

      const current = pointerCurrentRef.current;
      const target = pointerTargetRef.current;
      const amount = Math.min(1, dt * pointerSmoothRef.current);
  ...
  useEffect(() => {
    ...
    if (color.trim()) allColors.push(color);

    const parsed = allColors
      .filter(Boolean)
      .slice(0, MAX_COLORS)
      .map(toVec3);

    for (let i = 0; i < MAX_COLORS; i += 1) {
      const target = material.uniforms.uColors.value[i];
      if (i < parsed.length) target.copy(parsed[i]);
